...
class Recalculator(ABC):
    ...
    def offset_ids_query(self, limit: int, offset: int) -> RecalculatorQuery:
        ...
        Will be run by each worker to get their scores_ap.
...
class SimpleRecalculator(Recalculator):
    ...
    def __init__(
        ...
        * str (one condition)
        ...
        q = "SELECT {} FROM scores_ap JOIN beatmaps USING(beatmap_md5) WHERE {} ORDER BY scores_ap.id DESC"
        super(SimpleRecalculator, self).__init__(
            ids_query=RecalculatorQuery(q.format("scores_ap.id AS id", conditions_str), parameters),
...
class ScoreIdsPool:
    ...
    def chunk(self, chunk_size: int) -> List[int]:
        ...
        self.logger.debug("Chunked {} scores_ap. Current scores in pool: {}".format(chunk_size, len(self.scores)))
...
class Worker:
    """
    A tomejerry worker. Recalculates pp for a set of scores_ap.
    """
    ...
    def recalculate_pp(self):
        ...
        if db_worker is None:
            self.logger.warning("Cannot fetch scores_ap. No database worker available!!")
            return
        ...
        try:
            ...
            for i, lw_score in enumerate(self.scores):
                ...
                cursor.execute(
                    "SELECT * FROM scores_ap JOIN beatmaps USING(beatmap_md5) WHERE scores_ap.id = %s LIMIT 1",
                    (lw_score.score_id,)
                )
    ...
    def save_recalculations(self):
        ...
        for i, lw_score in enumerate(self.scores):
            ...
            glob.db.execute("UPDATE scores_ap SET pp = %s WHERE id = %s LIMIT 1", (lw_score.pp, lw_score.score_id))
...
def main():
    ...
    # Get recalculator
    recalculators_gen = {
        "zero": lambda: SimpleRecalculator(("scores_ap.completed = 3", "pp = 0")),
        "recalc": lambda: SimpleRecalculator(("scores_ap.completed = 3",)),
        "mods": lambda: SimpleRecalculator(("scores_ap.completed = 3", "mods & %s > 0"), (args.mods,)),
        "id": lambda: SimpleRecalculator(("scores_ap.id = %s",), (args.id,)),
        "gamemode": lambda: SimpleRecalculator(("scores_ap.completed = 3", "scores_ap.play_mode = %s",), (args.gamemode,)),
        "userid": lambda: SimpleRecalculator(("scores_ap.completed = 3", "scores_ap.userid = %s",), (args.userid,)),
        "beatmapid": lambda: SimpleRecalculator(("scores_ap.completed = 3", "beatmaps.beatmap_id = %s",), (args.beatmapid,)),
        "fixstdhd": lambda: SimpleRecalculator(("scores_ap.completed = 3", "scores_ap.play_mode = 0", "scores_ap.mods & 8 > 0"))
    }
    recalculator = None
