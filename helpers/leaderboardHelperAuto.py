...
def update(userID, newScore, gameMode):
	...
	if userUtils.isAllowed(userID):
		...
		glob.redis.zadd("ripple:leaderboard_ap:{}".format(scoreUtils.readableGameMode(gameMode)), str(userID), str(newScore))
	else:
...
def updateCountry(userID, newScore, gameMode):
	...
	if userUtils.isAllowed(userID):
		...
		if country is not None and len(country) > 0 and country.lower() != "xx":
			...
			k = "ripple:leaderboard_ap:{}:{}".format(scoreUtils.readableGameMode(gameMode), country.lower())
