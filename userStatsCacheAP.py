...
class userStatsCacheAP:
	def get(self, userID, gameMode):
		...
		"""
		data = glob.redis.get("lets:user_stats_cache_auto:{}:{}".format(gameMode, userID))
		if data is None:
			# If data is not cached, cache it and call get function again
			log.debug("userStatsCacheAP miss")
			self.update(userID, gameMode)
			return self.get(userID, gameMode)

		log.debug("userStatsCacheAP hit")
