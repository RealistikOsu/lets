...
class personalBestCacheAP:
	def get(self, userID, fileMd5, country=False, friends=False, mods=-1):
		...
		try:
			...
			data = glob.redis.get("lets:personal_best_cache_auto:{}".format(userID))
			if data is None:
				raise cacheMiss()

			# Unpack cached data
			data = data.decode("utf-8").split("|")
			cachedpersonalBestRankAP = int(data[0])
			...
			# Cache hit
			log.debug("personalBestCacheAP hit")
			return cachedpersonalBestRankAP
		except cacheMiss:
			log.debug("personalBestCacheAP miss")
			return 0
	...
	def set(self, userID, rank, fileMd5, country=False, friends=False, mods=-1):
		...
		:return:
		"""
		glob.redis.set("lets:personal_best_cache_auto:{}".format(userID), "{}|{}|{}|{}|{}".format(rank, fileMd5, country, friends, mods), 1800)
		log.debug("personalBestCacheAP set")
